    ...
    <div class="row" >
        ...
        <div class="col-lg-10 pt-3 pb-3">
            ...
            <nav class="navbar navbar-light bg-light">
                <form class="form-inline" action="/customerContract?action=search" method="post">
...
<%--                    <th scope="col">Nothing 2</th>--%>
                    <th scope="col">Edit</th>
                    <th scope="col">Delete</th>
                </tr>
                </thead>
                <tbody>
                <c:forEach items="${contracts}" var="result">
                    <tr>
                        <td>${result.customer.customerId}</td>
                        <td>${result.customer.customerName}</td>
                        <td>${result.contractId}</td>
                        <td>${result.employee.employeeId}</td>
                        <td>${result.service.serviceId}</td>
                        <td>${result.service.serviceName}</td>
<%--                        <td>${result.attachService.attachServiceName}</td>--%>
                        <td>${result.attachServiceListId}</td>
                        <td>${result.attachServiceListName}</td>
                        <td>${result.totalMoney}</td>
                        <td><a class="btn btn-success" href="/service?action=edit&id=${result.service.serviceId}" role="button">Edit</a></td>
                        <td><a onclick="ondelete('${result.service.serviceId}','${result.service.serviceName}')" class="btn btn-danger"
